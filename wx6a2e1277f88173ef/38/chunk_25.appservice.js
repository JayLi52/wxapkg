$gwx_XC_33 = function(_, _v, _n, _p, _s, _wp, _wl, $gwn, $gwl, $gwh, wh, $gstack, $gwrt, gra, grb, TestTest, wfor, _ca, _da, _r, _rz, _o, _oz, _1, _1z, _2, _2z, _m, _mz, nv_getDate, nv_getRegExp, nv_console, nv_parseInt, nv_parseFloat, nv_isNaN, nv_isFinite, nv_decodeURI, nv_decodeURIComponent, nv_encodeURI, nv_encodeURIComponent, $gdc, nv_JSON, _af, _gv, _ai, _grp, _gd, _gapi, $ixc, _ic, _w, _ev, _tsd) {
    return function(path, global) {
        if (typeof global === 'undefined') {
            if (typeof __GWX_GLOBAL__ === 'undefined') global = {};
            else global = __GWX_GLOBAL__;
        }
        if (typeof __WXML_GLOBAL__ === 'undefined') {
            __WXML_GLOBAL__ = {};
        }
        __WXML_GLOBAL__.modules = __WXML_GLOBAL__.modules || {};
        var e_ = {}
        if (typeof(global.entrys) === 'undefined') global.entrys = {};
        e_ = global.entrys;
        var d_ = {}
        if (typeof(global.defines) === 'undefined') global.defines = {};
        d_ = global.defines;
        var f_ = {}
        if (typeof(global.modules) === 'undefined') global.modules = {};
        f_ = global.modules || {};
        var p_ = {}
        __WXML_GLOBAL__.ops_cached = __WXML_GLOBAL__.ops_cached || {}
        __WXML_GLOBAL__.ops_set = __WXML_GLOBAL__.ops_set || {};
        __WXML_GLOBAL__.ops_init = __WXML_GLOBAL__.ops_init || {};
        var z = __WXML_GLOBAL__.ops_set.$gwx_XC_33 || [];

        function gz$gwx_XC_33_1() {
            if (__WXML_GLOBAL__.ops_cached.$gwx_XC_33_1) return __WXML_GLOBAL__.ops_cached.$gwx_XC_33_1
            __WXML_GLOBAL__.ops_cached.$gwx_XC_33_1 = [];
            (function(z) {
                var a = 11;

                function Z(ops) {
                    z.push(ops)
                }
                Z([
                    [7],
                    [3, 'p']
                ])
                Z([
                    [4],
                    [
                        [5],
                        [
                            [5],
                            [
                                [5],
                                [1, 'u-avatar']
                            ],
                            [1, 'data-v-d8fda593']
                        ],
                        [
                            [7],
                            [3, 'm']
                        ]
                    ]
                ])
                Z([
                    [2, '+'],
                    [
                        [2, '+'],
                        [
                            [7],
                            [3, 'n']
                        ],
                        [1, ';']
                    ],
                    [
                        [7],
                        [3, 'o']
                    ]
                ])
                Z([
                    [6],
                    [
                        [7],
                        [3, '$slots']
                    ],
                    [3, 'd']
                ])
                Z([
                    [7],
                    [3, 'a']
                ])
                Z([
                    [7],
                    [3, 'c']
                ])
                Z([
                    [7],
                    [3, 'd']
                ])
                Z([3, '__l'])
                Z([3, 'data-v-d8fda593'])
                Z([3, 'd8fda593-0'])
                Z([
                    [7],
                    [3, 'e']
                ])
                Z([
                    [7],
                    [3, 'f']
                ])
                Z(z[7])
                Z(z[8])
                Z([3, 'd8fda593-1'])
                Z([
                    [7],
                    [3, 'g']
                ])
            })(__WXML_GLOBAL__.ops_cached.$gwx_XC_33_1);
            return __WXML_GLOBAL__.ops_cached.$gwx_XC_33_1
        }
        __WXML_GLOBAL__.ops_set.$gwx_XC_33 = z;
        __WXML_GLOBAL__.ops_init.$gwx_XC_33 = true;
        var x = ['./node-modules/uview-plus/components/u-avatar/u-avatar.wxml'];
        d_[x[0]] = {}
        var m0 = function(e, s, r, gg) {
            var z = gz$gwx_XC_33_1()
            var hO9 = _mz(z, 'view', ['bindtap', 0, 'class', 1, 'style', 1], [], e, s, gg)
            var oP9 = _v()
            _(hO9, oP9)
            if (_oz(z, 3, e, s, gg)) {
                oP9.wxVkey = 1
                var cQ9 = _n('slot')
                _(oP9, cQ9)
            } else {
                oP9.wxVkey = 2
                var oR9 = _v()
                _(oP9, oR9)
                if (_oz(z, 4, e, s, gg)) {
                    oR9.wxVkey = 1
                }
                var lS9 = _v()
                _(oP9, lS9)
                if (_oz(z, 5, e, s, gg)) {
                    lS9.wxVkey = 1
                } else if (_oz(z, 6, e, s, gg)) {
                    lS9.wxVkey = 2
                    var aT9 = _mz(z, 'up-icon', ['bind:__l', 7, 'class', 1, 'uI', 2, 'uP', 3], [], e, s, gg)
                    _(lS9, aT9)
                } else if (_oz(z, 11, e, s, gg)) {
                    lS9.wxVkey = 3
                    var tU9 = _mz(z, 'up-text', ['bind:__l', 12, 'class', 1, 'uI', 2, 'uP', 3], [], e, s, gg)
                    _(lS9, tU9)
                } else {
                    lS9.wxVkey = 4
                }
                oR9.wxXCkey = 1
                lS9.wxXCkey = 1
                lS9.wxXCkey = 3
                lS9.wxXCkey = 3
            }
            oP9.wxXCkey = 1
            oP9.wxXCkey = 3
            _(r, hO9)
            return r
        }
        e_[x[0]] = {
            f: m0,
            j: [],
            i: [],
            ti: [],
            ic: []
        }
        if (path && e_[path]) {
            return function(env, dd, global) {
                $gwxc = 0;
                var root = {
                    "tag": "wx-page"
                };
                root.children = [];
                g = "$gwx_XC_33";
                var main = e_[path].f
                if (typeof global === "undefined") global = {};
                global.f = $gdc(f_[path], "", 1);
                try {
                    main(env, {}, root, global);
                    _tsd(root)
                } catch (err) {
                    console.log(err)
                };
                g = "";
                return root;
            }
        }
    }
}(__g.a, __g.b, __g.c, __g.d, __g.e, __g.f, __g.g, __g.h, __g.i, __g.j, __g.k, __g.l, __g.m, __g.n, __g.o, __g.p, __g.q, __g.r, __g.s, __g.t, __g.u, __g.v, __g.w, __g.x, __g.y, __g.z, __g.A, __g.B, __g.C, __g.D, __g.E, __g.F, __g.G, __g.H, __g.I, __g.J, __g.K, __g.L, __g.M, __g.N, __g.O, __g.P, __g.Q, __g.R, __g.S, __g.T, __g.U, __g.V, __g.W, __g.X, __g.Y, __g.Z, __g.aa);
if (__vd_version_info__.delayedGwx || false) $gwx_XC_33();
if (__vd_version_info__.delayedGwx) __wxAppCode__['node-modules/uview-plus/components/u-avatar/u-avatar.wxml'] = [$gwx_XC_33, './node-modules/uview-plus/components/u-avatar/u-avatar.wxml'];
else __wxAppCode__['node-modules/uview-plus/components/u-avatar/u-avatar.wxml'] = $gwx_XC_33('./node-modules/uview-plus/components/u-avatar/u-avatar.wxml');;
__wxRoute = "node-modules/uview-plus/components/u-avatar/u-avatar";
__wxRouteBegin = true;
__wxAppCurrentFile__ = "node-modules/uview-plus/components/u-avatar/u-avatar.js";
define("node-modules/uview-plus/components/u-avatar/u-avatar.js", function(require, module, exports, window, document, frames, self, location, navigator, localStorage, history, Caches, screen, alert, confirm, prompt, XMLHttpRequest, WebSocket, Reporter, webkit, WeixinJSCore) {
    "use strict";
    var A = require("../../../../common/vendor.js"),
        t = {
            name: "u-avatar",
            mixins: [A.mpMixin, A.mixin, A.props$7],
            data: function() {
                return {
                    colors: ["#ffb34b", "#f2bba9", "#f7a196", "#f18080", "#88a867", "#bfbf39", "#89c152", "#94d554", "#f19ec2", "#afaae4", "#e1b0df", "#c38cc1", "#72dcdc", "#9acdcb", "#77b1cc", "#448aca", "#86cefa", "#98d1ee", "#73d1f1", "#80a7dc"],
                    avatarUrl: this.src,
                    allowMp: !1
                }
            },
            watch: {
                src: {
                    immediate: !0,
                    handler: function(A) {
                        this.avatarUrl = A, A || this.errorHandler()
                    }
                }
            },
            computed: {
                imageStyle: function() {
                    return {}
                }
            },
            created: function() {
                this.init()
            },
            emits: ["click"],
            methods: {
                addStyle: A.addStyle,
                addUnit: A.addUnit,
                random: A.random,
                init: function() {
                    this.allowMp = !0
                },
                isImg: function() {
                    return -1 !== this.src.indexOf("/")
                },
                errorHandler: function() {
                    this.avatarUrl = this.defaultUrl || "data:image/jpg;base64,"
                },
                clickHandler: function(A) {
                    this.$emit("click", this.name, A)
                }
            }
        };
    Array || (A.resolveComponent("up-icon") + A.resolveComponent("up-text"))(), Math || (function() {
        return "../u-icon/u-icon.js"
    } + function() {
        return "../u-text/u-text.js"
    })();
    var a = A._export_sfc(t, [
        ["render", function(t, a, e, c, n, r) {
            return A.e({
                a: t.mpAvatar && n.allowMp
            }, t.mpAvatar && n.allowMp ? {
                b: A.s({
                    width: r.addUnit(t.size),
                    height: r.addUnit(t.size)
                })
            } : {}, {
                c: t.mpAvatar && n.allowMp
            }, t.mpAvatar && n.allowMp ? {} : t.icon ? {
                e: A.p({
                    name: t.icon,
                    size: t.fontSize,
                    color: t.color
                })
            } : t.text ? {
                g: A.p({
                    text: t.text,
                    size: t.fontSize,
                    color: t.color,
                    align: "center",
                    customStyle: "justify-content: center"
                })
            } : {
                h: A.n("u-avatar__image--".concat(t.shape)),
                i: n.avatarUrl || t.defaultUrl,
                j: t.mode,
                k: A.o((function() {
                    return r.errorHandler && r.errorHandler.apply(r, arguments)
                })),
                l: A.s({
                    width: r.addUnit(t.size),
                    height: r.addUnit(t.size)
                })
            }, {
                d: t.icon,
                f: t.text,
                m: A.n("u-avatar--".concat(t.shape)),
                n: A.s({
                    backgroundColor: t.text || t.icon ? t.randomBgColor ? n.colors["" !== t.colorIndex ? t.colorIndex : r.random(0, 19)] : t.bgColor : "transparent",
                    width: r.addUnit(t.size),
                    height: r.addUnit(t.size)
                }),
                o: A.s(r.addStyle(t.customStyle)),
                p: A.o((function() {
                    return r.clickHandler && r.clickHandler.apply(r, arguments)
                }))
            })
        }],
        ["__scopeId", "data-v-d8fda593"]
    ]);
    wx.createComponent(a);
}, {
    isPage: false,
    isComponent: true,
    currentFile: 'node-modules/uview-plus/components/u-avatar/u-avatar.js'
});
require("node-modules/uview-plus/components/u-avatar/u-avatar.js");